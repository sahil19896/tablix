...
tablix.change_request.ChangeRequest = Class.extend({
	...
	refresh: function(frm){
		...
		var me = this;
		if(!this.frm.is_new()){
			var name = String(this.frm.doc.for_document +"-"+ this.frm.doc.for_document_name);
		...
		if(!this.frm.doc.requested_by){
			this.frm.set_value("requested_by", frappe.session.user_email);
		}
	},
		...
		frappe.model.with_doctype(me.frm.doc.for_document, function(){
			...
			for(var i=0;i<fields.length; i++){
				...
				if(in_list(["Section Break", "Colum Break", "Image", "Text", "Text Editor",
				 	"Password", "Attach Image", "Attach", "Table", "Read Only", "Currency"],  field.fieldtype)){
					continue;
				}
	...
	company: function(){
		var me = this;
		if(this.frm.doc.company == "Tablix Technology LLC"){
			this.frm.set_value("approver", "kartik@tablix.ae");
		}else if(this.frm.doc.company == "Tablix Technologies Pvt. Ltd."){
			this.frm.set_value("approver", "gopu@tablix.ae");
		}else{
			this.frm.set_value("approver", "");
		}
	}
});
