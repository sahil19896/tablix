...
function send_document(frm){
	if(cur_frm.doc.tablix_status == "Approved"){
		var table = cur_frm.doc.employee || "";
		if(table){
			for(var i=0; i<table.length; i++){
				if(frappe.session.user_email == table[i].email){
					cur_frm.toggle_display("approver");
					cur_frm.toggle_display("description");
					cur_frm.toggle_display("employee");
					cur_frm.toggle_display("department");
					cur_frm.add_custom_button(__("View Notification"), function() {
						frappe.set_route("notification", {
...
frappe.ui.form.on('Notification', {
	before_save: function(frm) {
		if(frappe.session.user_email == cur_frm.doc.request_by){
			var table = cur_frm.doc.approver;
			for(var i=0; i<table.length; i++){
		...
		if(cur_frm.doc.tablix_status == "Waiting For Approval"){
			cur_frm.assign_to.remove(cur_frm.doc.request_by);
		}
	}
});
...
frappe.ui.form.on('Notification', {
	onload_post_render: function(frm){
		...
		if(! cur_frm.doc.request_by){
			cur_frm.set_value("request_by", frappe.session.user_email);
		}

		if(! cur_frm.doc.approver){
			for(var i=0;i<approver.length;i++){
				var child = frappe.model.get_new_doc("Policy Approvers",cur_frm.doc,"approver");
				$.extend(child, {
					"employee": approver[i],
...
function check_approval(cur_frm){
	if(cur_frm.doc.tablix_status == "Waiting For Approval"){
		...
		if(accept+decline == table.length){
			if(decline > 0){
				frappe.model.set_value("Notification", cur_frm.doc.name, "tablix_status", "Open");
			} else if(accept == table.length){
				frappe.model.set_value("Notification", cur_frm.doc.name, "tablix_status", "Completed");
			}
		}
	}
}
...
frappe.ui.form.on("Notification", "all_employee", function(frm){
	if(cur_frm.doc.all_employee == 1){
		cur_frm.clear_table("employee");
			...
			callback: function(r){
				if(r && r.message){
					...
					for(var i=0;i<emps.length;i++){
						var child = frappe.model.get_new_doc("Policy Employee",cur_frm.doc,"employee");
						$.extend(child, {
...
frappe.ui.form.on("Notification", "departments", function(frm){
        if(cur_frm.doc.departments == "HR"){
                frappe.model.set_value("Notification", cur_frm.doc.name, "naming_series", "NT-HR-");
        } else if(cur_frm.doc.departments == "Finance"){
                frappe.model.set_value("Notification", cur_frm.doc.name, "naming_series", "NT-FA-");
        } else if(cur_frm.doc.departments == "Project Management"){
                frappe.model.set_value("Notification", cur_frm.doc.name, "naming_series", "NT-PM-");
        } else if(cur_frm.doc.departments == "Pre-Sales"){
                frappe.model.set_value("Notification", cur_frm.doc.name, "naming_series", "NT-PS-");
        } else if(cur_frm.doc.departments == "Purchase"){
                frappe.model.set_value("Notification", cur_frm.doc.name, "naming_series", "NT-PR-");
        }
});
