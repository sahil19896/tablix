...
class TablixMeta(Meta):
	def __init__(self, doctype):
		super(TablixMeta, self).__init__(doctype)
	...
	def as_dict(self, no_nulls=False):
		d = super(TablixMeta, self).as_dict(no_nulls=no_nulls)
	...
	def add_code(self):
		...
		if not hooks.get(self._doctype): 
	...
	def load_dashboard(self):
		...
		temp = self.load_dashboard_hooks()
		if temp:
			self.set('__dashboard', temp)
		else:
			self.set('__dashboard', self.get_dashboard_data())


	def load_dashboard_hooks(self):
		hooks = frappe.get_hooks("load_doctype_meta")
		dashboard_data = None
		dashboards = hooks.get(self._doctype)  or []
		for dashboard in dashboards:
			try:
				module = import_module(dashboard)
				if hasattr(module, "get_data"):
					dashboard_data  = module.get_data()
